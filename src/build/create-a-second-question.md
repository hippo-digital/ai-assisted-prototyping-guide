...
<div class="code-block"><code>On app/views/issue-location.html, the hint text is not showing. Make sure each hint uses the govuk-hint class and is placed between the label and the input inside the govuk-form-group.</code><button class="code-copy-btn" aria-label="Copy to clipboard">Copy</button></div>
</div>
</details>
...
<div class="code-block"><code>Update the textarea on app/views/issue-location.html to have rows="8" to give users more space to write.</code><button class="code-copy-btn" aria-label="Copy to clipboard">Copy</button></div>
</div>
</details>
...
<div class="code-block"><code>In app/routes.js, add validation for the /issue-location form. If either the issue-floor or issue-description fields are empty, redirect back to /issue-location and show an error message on the relevant field using GOV.UK error message markup.</code><button class="code-copy-btn" aria-label="Copy to clipboard">Copy</button></div>

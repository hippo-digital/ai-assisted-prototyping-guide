...
Before touching routes.js, it is worth asking Gemini to verify that all your pages are pointing to the right URLs. A mismatch here is the most common reason a journey skips pages or ends up in the wrong place.

```
Check the form action attributes and button links across all pages in app/views. They should follow this pattern — each form posts to its own URL, and the router handles the redirect to the next page:
...
Fix any that are wrong and tell me what you changed.
```
...
```
Update app/routes.js to add routing for the "Report a building issue" prototype journey.
...
Do not remove any existing code. Do not add any GET routes.
```

Type **yes** when Gemini asks permission to edit the file.
